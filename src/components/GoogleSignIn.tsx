...
import { trackUserSignIn } from '@/lib/analytics';
...
export const GoogleSignIn = ({ onSuccess }: GoogleSignInProps) => {
        ...
        onSuccess: async (tokenResponse) => {
            try {
                ...
                toast({
                    ...
                    description: error.response?.data?.message || 'فشل تسجيل الدخول عبر Google',
                    variant: 'destructive',
                });
            }
        },
    ...
    return (
        <Button
            type="button"
            variant="outline"
            className="w-full relative overflow-hidden group border-2 hover:border-primary transition-all duration-300"
            onClick={() => login()}
        >
